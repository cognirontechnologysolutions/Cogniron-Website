...
import { CheckCircle2, Zap, Settings, GitBranch, ArrowRight } from 'lucide-react';
import { PageContainer } from '../../../components/PageContainer';
import { useTheme } from '../../../contexts/ThemeContext';
import { ServicePill } from '../../../components/ServicePill';

export function XFirstDevOpsToolChainPage() {
  const { theme } = useTheme();

  const relatedServices = [
    { icon: Zap, title: 'Test Automation', description: 'Automated test execution', gradient: 'from-blue-500 to-cyan-400', link: '/services/x-first/test-automation' }
  ];

  const capabilities = ['End-to-end CI/CD pipeline automation', 'Infrastructure as Code (IaC) implementation', 'Container orchestration and management', 'Automated deployment pipelines', 'Integrated monitoring and observability', 'Toolchain optimization and integration'];
  const processSteps = [
    { title: 'Assess', description: 'Evaluate current tooling state' },
    { title: 'Design', description: 'Plan DevOps architecture' },
    { title: 'Implement', description: 'Integrate and configure toolchain' },
    { title: 'Automate', description: 'Build automated pipelines' },
    { title: 'Optimize', description: 'Continuously refine and scale' }
  ];
  const tools = ['Jenkins', 'GitLab', 'Docker', 'Kubernetes', 'Terraform', 'Ansible'];
  const metrics = [
    { value: '80%', label: 'Time Saved', sublabel: '' },
    { value: '10x', label: 'Deployment Frequency', sublabel: '' },
    ...
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>DevOps Tool Chain (TEM/TDM) <span style={{ fontSize: '0.5em' }}>(Tooling-first enablement)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Enable development speed with unified CI/CD tooling. Save 80% deployment time, achieve 10x deployment frequency, and 99.9% infrastructure uptime.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>Integrate testing, deployment, and monitoring through Infrastructure as Code and container orchestration.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                <GitBranch className="w-16 h-16 text-cyan-500 mb-4" />
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Complete Automation</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>CI/CD tooling with Infrastructure as Code. Deployment, testing, monitoring—unified and automated.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">80%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Time saved</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">10x</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Frequency</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(34, 211, 238, 0.2), rgba(59, 130, 246, 0.2))' : 'linear-gradient(135deg, rgba(34, 211, 238, 0.1), rgba(59, 130, 246, 0.1))', color: '#22D3EE', border: `1px solid ${theme === 'dark' ? 'rgba(34, 211, 238, 0.3)' : 'rgba(34, 211, 238, 0.2)'}` }}>Enterprise SaaS</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Enterprise SaaS</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Saved 80% deployment time, achieved 10x deployment frequency, and 99.9% infrastructure uptime through unified DevOps tooling.</p>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-5xl mb-6" style={{ color: 'var(--text-primary)' }}>Build Faster with <span className="gradient-text">DevOps</span></h2>
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Enable speed and reliability with tooling-first automation.</p>
