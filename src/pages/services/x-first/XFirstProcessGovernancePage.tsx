...
export function XFirstProcessGovernancePage() {
  const { theme } = useTheme();

  const relatedServices = [
    { icon: Shield, title: 'Security Engineering', description: 'Security compliance gates', gradient: 'from-cyan-400 to-blue-500', link: '/services/x-first/security-engineering' }
  ];

  const capabilities = ['CI/CD-embedded quality gates', 'Policy-driven test enforcement', 'Compliance and standards validation', 'Coverage and release criteria checks', 'Automated audit trails and traceability'];
  const processSteps = [
    { title: 'Define', description: 'Set quality and compliance policies' },
    { title: 'Automate', description: 'Embed gates in CI/CD' },
    { title: 'Enforce', description: 'Block non-compliant releases' },
    { title: 'Monitor', description: 'Track compliance metrics' },
    { title: 'Optimize', description: 'Continuously refine governance' }
  ];
  const tools = ['SonarQube', 'Checkmarx', 'Xray', 'TestRail', 'qTest', 'Zephyr'];
  const metrics = [
    { value: '100%', label: 'Compliance', sublabel: '' },
    { value: '-90%', label: 'Manual Reviews', sublabel: '' },
    { value: '0', label: 'Non-Compliant Releases', sublabel: '' }
  ];
    ...
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>Process Governance <span style={{ fontSize: '0.5em' }}>(Quality-first practices embedded)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Automate quality governance with CI/CD-embedded controls. Enforce standards at every stage and block non-compliant releases through policy-driven quality gates.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>Achieve 100% policy compliance and reduce manual reviews by 90% with automated enforcement and continuous auditability.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                ...
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Governance, Built In</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Quality gates run automatically in your pipelines. Compliance enforced without manual overhead.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">100%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Compliance</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">-90%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Reviews</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(34, 211, 238, 0.2), rgba(59, 130, 246, 0.2))' : 'linear-gradient(135deg, rgba(34, 211, 238, 0.1), rgba(59, 130, 246, 0.1))', color: '#22D3EE', border: `1px solid ${theme === 'dark' ? 'rgba(34, 211, 238, 0.3)' : 'rgba(34, 211, 238, 0.2)'}` }}>Fintech Company</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>FinTech Company</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Achieved 100% policy compliance, reduced manual reviews by 90%, and blocked all non-compliant releases through automated governance.</p>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-5xl mb-6" style={{ color: 'var(--text-primary)' }}>Enforce Quality by <span className="gradient-text">Design</span></h2>
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Embed quality governance directly into your delivery pipelines.</p>
