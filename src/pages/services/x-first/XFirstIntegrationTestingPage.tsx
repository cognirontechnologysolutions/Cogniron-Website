...
import { CheckCircle2, GitMerge, ArrowRight } from 'lucide-react';
import { PageContainer } from '../../../components/PageContainer';
import { useTheme } from '../../../contexts/ThemeContext';
import { ServicePill } from '../../../components/ServicePill';

export function XFirstIntegrationTestingPage() {
  const { theme } = useTheme();

  const relatedServices = [
    { icon: CheckCircle2, title: 'XFirst TestOps', description: 'Pipeline-first orchestration', gradient: 'from-cyan-500 to-blue-600', link: '/services/x-first/xfirst-testops' }
  ];

  const capabilities = ['CI-integrated integration testing', 'API and microservices validation', 'Interface contract testing', 'Service mesh and inter-service testing', 'Database integration verification', 'Third-party integration testing'];
  const processSteps = [
    { title: 'Integrate', description: 'Code merge triggers validation' },
    { title: 'Contract', description: 'Verify interface agreements' },
    { title: 'Test', description: 'Run integration test suites' },
    { title: 'Feedback', description: 'Immediate results to teams' },
    { title: 'Fix', description: 'Resolve issues fast' }
  ];
  const tools = ['Postman', 'SoapUI', 'Pact', 'WireMock', 'REST Assured', 'Karate'];
  const metrics = [
    { value: '90%', label: 'Issues Caught', sublabel: '' },
    { value: '-75%', label: 'Integration Bugs', sublabel: '' },
    { value: '<30min', label: 'Detection', sublabel: '' }
  ];
    ...
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>Integration Testing <span style={{ fontSize: '0.5em' }}>(Early service interaction validation)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Continuously validate integrations with every code merge. Catch API, microservices, and interface issues early through automated integration testing in CI.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>Detect 90% of interface issues within 30 minutes, reducing integration bugs by 75% before they reach production.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                <GitMerge className="w-16 h-16 text-cyan-500 mb-4" />
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Always-On Integration Validation</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Test integration points automatically. Stop interface failures before they cascade.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">90%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Caught</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">&lt;30m</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Detection</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(34, 211, 238, 0.2), rgba(59, 130, 246, 0.2))' : 'linear-gradient(135deg, rgba(34, 211, 238, 0.1), rgba(59, 130, 246, 0.1))', color: '#22D3EE', border: `1px solid ${theme === 'dark' ? 'rgba(34, 211, 238, 0.3)' : 'rgba(34, 211, 238, 0.2)'}` }}>Microservices Platform</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Microservices Platform</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Detected 90% of interface issues immediately and reduced production integration bugs by 75% through continuous integration testing.</p>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Validate integration points continuously with automated testing</p>
