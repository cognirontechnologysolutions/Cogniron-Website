...
import { CheckCircle2, Shield, Lock, AlertTriangle, ArrowRight } from 'lucide-react';
import { PageContainer } from '../../../components/PageContainer';
import { useTheme } from '../../../contexts/ThemeContext';
import { ServicePill } from '../../../components/ServicePill';

export function SecurityEngineeringPage() {
  const { theme } = useTheme();

  const relatedServices = [
    { icon: AlertTriangle, title: 'Cognitive Security Testing', description: 'AI-driven threat detection and risk prediction', gradient: 'from-pink-500 to-purple-600', link: '/services/cognitive-assurance/cognitive-security-testing' },
    { icon: Shield, title: 'SIEM Cognitive Correlation', description: 'Advanced security event intelligence and correlation', gradient: 'from-cyan-400 to-blue-500', link: '/services/cognitive-assurance/siem' },
    { icon: Lock, title: 'X-First Process Governance', description: 'Security controls embedded into delivery workflows', gradient: 'from-blue-500 to-cyan-400', link: '/services/x-first/xfirst-process-governance' }
  ];

  const capabilities = ['Static & Dynamic Application Security Testing (SAST/DAST)', 'Vulnerability assessment and penetration testing', 'Secure code review and threat modeling', 'Cloud, container, and Kubernetes security validation', 'API and microservices security testing', 'Compliance assurance (OWASP, PCI-DSS, SOC 2)'];
  const processSteps = [
    { title: 'Threat Modeling', description: 'Identify and prioritize security risks' },
    { title: 'Static Analysis', description: 'Detect code-level vulnerabilities early' },
    { title: 'Dynamic Testing', description: 'Validate runtime behavior and attacks' },
    { title: 'Penetration Testing', description: 'Simulate real-world threats' },
    { title: 'Remediation & Hardening', description: 'Verify fixes and prevent regression' }
  ];
  const tools = ['Snyk', 'SonarQube', 'OWASP ZAP', 'Burp Suite', 'Checkmarx', 'Veracode'];
  const metrics = [
    { value: '95%', label: 'Vulnerabilities removed pre-production', sublabel: '' },
    { value: '100%', label: 'OWASP Top 10 compliance', sublabel: '' },
    { value: '24h', label: 'Critical fix response time', sublabel: '' }
  ];
  ...
  return (
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>Security Engineering <span className="text-2xl lg:text-3xl">(secure-by-design / early threat modeling)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Shift security left with secure-by-design engineering and early threat modeling. Cogniron embeds security controls from architecture to deployment, eliminating 95% of vulnerabilities before production through integrated SAST/DAST and continuous security validation.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>Achieve 100% OWASP Top 10 compliance with 24-hour critical fix response, ensuring security is engineered—not retrofitted—into every release.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                ...
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Secure by Design</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Threat modeling from day one. Continuous security validation embedded into CI/CD pipelines prevents vulnerabilities from ever reaching production.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">95%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Vulnerabilities eliminated pre-production</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">100%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Standards compliance</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ background: theme === 'dark' ? 'linear-gradient(180deg, rgba(15, 23, 42, 1) 0%, rgba(30, 41, 59, 1) 50%, rgba(15, 23, 42, 1) 100%)' : 'linear-gradient(180deg, rgba(249, 250, 251, 1) 0%, rgba(241, 245, 249, 1) 50%, rgba(249, 250, 251, 1) 100%)' }}>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-4xl md:text-5xl text-center mb-12" style={{ color: 'var(--text-primary)' }}><span className="gradient-text">Core Security Capabilities</span></h2>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-4xl md:text-5xl mb-12" style={{ color: 'var(--text-primary)' }}><span className="gradient-text">Our Shift-Left Security Model</span></h2>
          <div className="relative">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <p className="text-sm text-center" style={{ color: 'var(--text-secondary)' }}>
              Additional tools selected based on architecture and compliance needs
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(34, 211, 238, 0.2), rgba(59, 130, 246, 0.2))' : 'linear-gradient(135deg, rgba(34, 211, 238, 0.1), rgba(59, 130, 246, 0.1))', color: '#22D3EE', border: `1px solid ${theme === 'dark' ? 'rgba(34, 211, 238, 0.3)' : 'rgba(34, 211, 238, 0.2)'}` }}>Healthcare Platform</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>95% Vulnerability Reduction with Secure-by-Design Engineering</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>A healthcare SaaS platform embedded shift-left security across development, eliminating critical vulnerabilities before release and maintaining full compliance across regulated environments.</p>
              <div className="grid grid-cols-3 gap-6 pt-6" style={{ borderTop: '1px solid var(--border-color)' }}>
                {metrics.map((metric, index) => (
                  <div key={index} className="text-center">
                    <div className="text-3xl gradient-text mb-2" dangerouslySetInnerHTML={{ __html: metric.value }}></div>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-5xl mb-6" style={{ color: 'var(--text-primary)' }}>Ready to Shift Security Left?</h2>
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Engineer security into every stage of delivery with our security practices.</p>
