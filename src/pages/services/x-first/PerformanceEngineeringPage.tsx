...
import { CheckCircle2, TrendingUp, Gauge, Zap, ArrowRight } from 'lucide-react';
import { PageContainer } from '../../../components/PageContainer';
import { useTheme } from '../../../contexts/ThemeContext';
import { ServicePill } from '../../../components/ServicePill';

export function PerformanceEngineeringPage() {
  const { theme } = useTheme();

  const relatedServices = [
    { icon: Zap, title: 'Test Automation', description: 'Automated performance regression in CI/CD', gradient: 'from-cyan-400 to-blue-500', link: '/services/x-first/test-automation' },
    { icon: TrendingUp, title: 'Cognitive Performance Testing', description: 'AI-driven anomaly detection and prediction', gradient: 'from-pink-500 to-purple-600', link: '/services/cognitive-assurance/cognitive-performance-testing' },
    { icon: CheckCircle2, title: 'TestOps', description: 'Performance metrics embedded in pipelines', gradient: 'from-blue-500 to-cyan-400', link: '/services/x-first/xfirst-testops' }
  ];

  const capabilities = ['Load, stress, and endurance testing', 'Performance profiling and bottleneck isolation', 'Scalability modeling and capacity planning', 'Real User Monitoring (RUM)', 'CI/CD performance gates and thresholds', 'Cloud-native performance optimization'];
  const processSteps = [
    { title: 'Baseline', description: 'Define performance SLAs and benchmarks' },
    { title: 'Modeling', description: 'Create realistic workload and traffic patterns' },
    { title: 'Testing', description: 'Execute load, stress, and scalability tests' },
    { title: 'Analysis', description: 'Identify bottlenecks and root causes' },
    { title: 'Optimization', description: 'Tune, validate, and realign improvements' }
  ];
  const tools = ['JMeter', 'K6', 'Gatling', 'Locust', 'New Relic', 'Datadog'];
  const metrics = [
    { value: '50%', label: 'Response time improvement', sublabel: '' },
    { value: '5x', label: 'Scalability increase', sublabel: '' },
    { value: '99.9%', label: 'Availability under peak load', sublabel: '' }
  ];
  ...
  return (
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>Performance Engineering <span className="text-2xl lg:text-3xl">(proactive design and early perf checks)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Shift performance left with proactive engineering and continuous validation. Cogniron embeds performance thinking from architecture through release, delivering 50% faster response times and 5x scalability using CI/CD-integrated performance gates.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>Sustain 99.9% availability under peak load by identifying and resolving performance risks long before they reach production.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                <Gauge className="w-16 h-16 text-cyan-500 mb-4" />
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Proactive Performance by Design</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>Build systems that perform under pressure. Early benchmarking, continuous validation, and real-world load modeling eliminate surprises in production.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">50%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Faster response times</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl gradient-text">5x</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Scalability achieved</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-4xl md:text-5xl mb-12" style={{ color: 'var(--text-primary)' }}><span className="gradient-text">Our Performance Engineering Model</span></h2>
          <div className="relative">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <p className="text-sm text-center" style={{ color: 'var(--text-secondary)' }}>
              Additional tools selected based on architecture and scale needs
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(34, 211, 238, 0.2), rgba(59, 130, 246, 0.2))' : 'linear-gradient(135deg, rgba(34, 211, 238, 0.1), rgba(59, 130, 246, 0.1))', color: '#22D3EE', border: `1px solid ${theme === 'dark' ? 'rgba(34, 211, 238, 0.3)' : 'rgba(34, 211, 238, 0.2)'}` }}>Financial Services Platform</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>50% Faster Response with 5x Scalability</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>A high-volume financial services platform embedded performance engineering early in delivery, enabling faster response times and sustained availability during peak transaction loads.</p>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-5xl mb-6" style={{ color: 'var(--text-primary)' }}>Ready to Engineer Performance In?</h2>
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Shift left, scale confidently, and deliver consistent performance with Cogniron's X-First performance engineering.</p>
