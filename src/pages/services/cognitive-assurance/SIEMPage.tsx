...
import { CheckCircle2, Shield, Activity, ArrowRight } from 'lucide-react';
...
export function SIEMPage() {
  ...
  const relatedServices = [
    { icon: Shield, title: 'Security Testing', description: 'AI-driven vulnerability correlation and risk prioritization', gradient: 'from-purple-500 to-pink-500', link: '/services/cognitive-assurance/cognitive-security-testing' },
    { icon: Activity, title: 'Production Monitoring', description: 'AI-based observability and anomaly detection', gradient: 'from-pink-500 to-purple-500', link: '/services/cognitive-assurance/ai-production-monitoring' }
  ];

  const capabilities = ['Cognitive correlation across events', 'Intelligent alert prioritization based on context', 'AI-driven threat hunting and behavioral analysis', 'Automated classification and severity scoring', 'Predictive threat detection using ML models', 'Smart false-positive filtering and suppression'];
  const processSteps = [
    { title: 'Collect', description: 'Aggregate security events across infrastructure' },
    { title: 'Correlate', description: 'AI identifies hidden patterns and attack signals' },
    { title: 'Classify', description: 'Incidents are categorized by type and severity' },
    { title: 'Prioritize', description: 'Threats ranked based on exploitability and impact' },
    { title: 'Respond', description: 'Guided response workflows triggered' }
  ];
  const tools = ['Splunk', 'ELK Stack', 'QRadar', 'Azure Sentinel', 'TensorFlow', 'Scikit-learn'];
  const metrics = [
    { value: '85%', label: 'Alert noise reduced', sublabel: '' },
    { value: '10x', label: 'Faster threat triage', sublabel: '' },
    { value: '99%', label: 'Threat detection coverage', sublabel: '' }
  ];
    ...
    <PageContainer>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl mb-6" style={{ color: 'var(--text-primary)' }}>SIEM <span style={{ fontSize: '0.5em' }}>(cognitive correlation and alert intelligence)</span></h1>
              <p className="text-xl leading-relaxed mb-6" style={{ color: 'var(--text-secondary)' }}>Transform SIEM(Security Information and Event Management) from alert-heavy monitoring to intelligent threat intelligence. Cogniron's SIEM Cognitive Correlation applies AI-driven correlation to reduce alerts by 85%, accelerate triage 10x, and maintain 99% threat detection coverage across complex environments.</p>
              <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>By correlating events across systems, predicting attack paths, and automating incident classification, security teams gain clarity, speed, and confidence in every response.</p>
            ...
            <div className="relative">
              ...
              <div className="relative backdrop-blur-sm rounded-3xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                ...
                <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>Cognitive Intelligence</h3>
                <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>AI correlates signals across logs, events, and telemetry to eliminate noise and prioritize threats based on real-world risk impact.</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">85%</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Alert reduction</div></div>
                  <div className="rounded-lg p-4" style={{ backgroundColor: theme === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(241, 245, 249, 1)' }}><div className="text-3xl bg-gradient-to-r from-pink-500 to-purple-600 bg-clip-text text-transparent">10x</div><div className="text-sm" style={{ color: 'var(--text-secondary)' }}>Faster triage</div></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-20" style={{ background: theme === 'dark' ? 'linear-gradient(180deg, rgba(15, 23, 42, 1) 0%, rgba(30, 41, 59, 1) 50%, rgba(15, 23, 42, 1) 100%)' : 'linear-gradient(180deg, rgba(249, 250, 251, 1) 0%, rgba(241, 245, 249, 1) 50%, rgba(249, 250, 251, 1) 100%)' }}>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl p-8" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <p className="text-sm text-center" style={{ color: 'var(--text-secondary)' }}>Custom-trained ML models for advanced security event correlation</p>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-sm rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <div className="p-8">
              <span className="inline-block px-4 py-2 text-sm rounded-lg mb-4" style={{ background: theme === 'dark' ? 'linear-gradient(135deg, rgba(236, 72, 153, 0.2), rgba(147, 51, 234, 0.2))' : 'linear-gradient(135deg, rgba(236, 72, 153, 0.1), rgba(147, 51, 234, 0.1))', color: '#EC4899', border: `1px solid ${theme === 'dark' ? 'rgba(236, 72, 153, 0.3)' : 'rgba(236, 72, 153, 0.2)'}` }}>Enterprise Security Operations</span>
              <h3 className="text-2xl mb-4" style={{ color: 'var(--text-primary)' }}>85% Alert Reduction with Cognitive SIEM Intelligence</h3>
              <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>A global enterprise security team adopted AI-driven SIEM correlation to eliminate alert fatigue, improve threat visibility, and dramatically accelerate incident response across distributed environments.</p>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-5xl mb-6" style={{ color: 'var(--text-primary)' }}>Ready to Modernize Your SIEM with <span className="gradient-text">Cognitive Intelligence</span>?</h2>
          <p className="text-xl mb-10 max-w-3xl mx-auto" style={{ color: 'var(--text-secondary)' }}>Reduce noise, surface real threats, and respond with speed using AI-driven SIEM correlation.</p>
